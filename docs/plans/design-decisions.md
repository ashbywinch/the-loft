...
| 2026-09-18 | `boxdet.py` / `boxsimple.py` / `boxscale.py` deleted | the dead predecessor of this pipeline ("boxdet's machinery dies" per boxsimple's own thesis); nothing imported them — the standing `duplicate` findings were same-origin debt, not a deliberate parallel | git history; docs/plans/vlm-word-segmentation-spike.md |
