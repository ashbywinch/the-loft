        ...
        out.append(
            {
                "page": str(entry["page"]),
                "text": str(entry["text"]),
                "starts_document": _flag_bool(entry.get("starts_document", False)),
                "ends_document": _flag_bool(entry.get("ends_document", False)),
    ...
    if not boundaries_path.exists():
        ...
        if classify_path.exists():
            ...
            if not any(entry.get("kind") == "text" for entry in classify.values()):
                # a photo-only batch has nothing to transcribe (2026-08-14
                # final review: `make confirm` must not error on it)
