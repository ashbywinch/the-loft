...
def split_shapes(shapes: list[Mark], lines: list[Line]) -> list[Mark]:
    ...
    the shape breaks where that assignment changes — but only where the ink
    narrows to a waist there. A boundary through full-width ink is the row
    fit's wobble, and the shape stays whole.
    """
    out: list[Mark] = []
    for shape in shapes:
        per_row = shape.rows()
