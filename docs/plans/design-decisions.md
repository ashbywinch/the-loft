...
| 2026-08-16 | The reviewer's own reorientation is authoritative and persists independently of the backend: instant + on every look thereafter (PRD VR10). A failed re-read says plainly that it failed and never blames a correction the reviewer didn't make | The first field test exposed two defects: a stale queued intent could re-orient a page the reviewer never touched and mark its re-read failed, and the view rotation accumulated (page-02 marched to 270°/landscape), so a fix the reviewer made appeared to revert | PRD VR10 + acceptance 11/12; ux-fixes-plan "Orientation fix" |
