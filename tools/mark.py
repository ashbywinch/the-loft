...
def _thick_zone(ys: np.ndarray, xs: np.ndarray, span: float) -> set[int]:
...
def baseline_row(ys: np.ndarray, xs: np.ndarray) -> int:
    ...
    if band_at_foot:
        # the LAST broad band touches the word's foot: an UNDERLINE owns the
        # bottom. The letters end at the last row above the band's deep
        # valley with real body density. The Opera box: the letters end at
        # row 43, the valley dips to 2-5, the underline's core runs 50-56.
        # The valley must be DEEP - a dense bottom is a bottom-heavy word's
        # own body, not an underline.
        band = bands[-1]
        band_top = min(band)
        valley = (
        ...
        if intensities[valley] <= 0.2 * intensities[peak_row]:
            ...
            return seam
    forbidden = _thick_zone(ys, xs, span)
    ...
    # the ±2 band modal (measured: ±4 drifts the centre; ±2 puts the f's
    # bodies at 39 against 41 and the g's at 23 against 21); ties go to the
    # lower row (a bar sits ABOVE the baseline).
    best, best_bottom = 0, 0
    for candidate in sorted(set(kept)):
        count = sum(1 for b in kept if abs(b - candidate) <= 2)
...
def _broad_waistline(ys: np.ndarray, xs: np.ndarray, width: float) -> int | None:
...
def waistline_row(ys: np.ndarray, xs: np.ndarray) -> int:
    ...
    broad = _broad_waistline(kept_ys, xs[keep], span)
