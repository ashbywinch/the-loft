...
def test_version_2_has_suffix(tmp_path: Path) -> None:
    ...
    store.write("test.json", "v1")
    store.write("test.json", "v2")
    assert (tmp_path / "test.json").read_text() == "v1"
    assert (tmp_path / "test-2.json").read_text() == "v2"
...
def test_write_never_overwrites(tmp_path: Path) -> None:
    ...
    assert store.read_latest("test.json") == "v2"
    assert (tmp_path / "test.json").read_text() == "v1"  # original preserved
...
def test_txt_files_are_versioned_too(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "guess.txt").read_text() == "first"
    assert (tmp_path / "guess-2.txt").read_text() == "second"
...
def test_paths_with_dots_in_stem(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "ocr-guess" / "page-02.layout.json").read_text() == "v1"
    assert (tmp_path / "ocr-guess" / "page-02.layout-2.json").read_text() == "v2"
