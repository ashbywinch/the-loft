...
- **Normative home:** `docs/TECHSPEC.md` §16 (draft). **Parity partner:** `docs/PRD/MEMORIES.md` (the story flow)
...
- **J — orientation is settled before any reading (2026-08-10, supersedes the model-first pipeline):** scans may be rotated; the reading is **never** attempted until the page is upright, and the orientation is settled **deterministically — never by a model's single guess** (every explicit orientation detector proved unreliable, 2026-08-10). Mechanism: `TECHSPEC.md` §16.
...
- **P — precision comes from geocoding, never from the place's type:** every proposed coordinate carries its precision (`exact | street | town | county | region | country | continent`). The flow **geocodes the specific place first** — by name and any address in the evidence — and proposes the precision the match honestly supports: a specific match (street-numbered address, a surveyed point) is `exact`; a street/square-level match is `street`; no specific match → the **research step (Rule S)** before any settlement fallback; and only when research finds nothing does the settlement-centre fallback apply at `town` (or the larger area's level). **You cannot know whether a named building is locatable until you look** — and "looking" includes the web, not just the geocoder: a building that no longer exists under its name may still stand under a successor's (2026-08-05, the Lark Inn: a blue-plaque search confirmed the building still stands on Market Square, Stonewick, as the Golden Lark/Walton's Yard, and a Places-API geocode of the surviving building pinned it exactly). The reviewer accepts or adjusts the point and the precision together, and an imprecise point renders as an uncertainty ring, never a pin.

### 2.4 Data-model requirements (2026-08-05)

- **Organisations are first-class** alongside people and places: an
  organisation can be a sender or recipient; it carries its written
  address and branch — the address is the organisation's, never a place
  record. An archive without organisations still works.
- **People carry honest dates of birth and death** (never a fake exact
  date) **and attested occupations** (forms as written).
- **Places carry their written address and their household** — who lived
  there and when (a deduction is proposed, the household rule).
- **Attested objects and attested items without scans are referenceable**
  (memorabilia); their evidence is named in provenance/description.
- **Referenced dates are recorded separately from the item's own date**
  (a letter mentioning a marriage on 27 Jan 1949 carries both).
- **Proposed people live in the people table**; relationships resolve
  against the table regardless of status.
- **Same-named people with distinct birth dates are distinct people**,
  never collapsed.
- **Dates express the bounds the artifacts actually state** — `before` /
  `after` / `between` alongside `exact` / `month` / `year` / `approx` (a
  year-only date is exact at year granularity, never uncertainty) — and
  survive export losslessly.
...
**Elicitation layer:** one deterministic prompt per stage (the `MEMORIES.md` discipline); the contract is `TECHSPEC.md` §16's.
...
9. Orientation is settled before any reading, deterministically; the transcript is checked/fixed before proposals; the item date is the earliest attestation; the origin/first-encounter question is asked.
...
- `docs/TECHSPEC.md` §16 — import flow design (draft; this PRD feeds it)
