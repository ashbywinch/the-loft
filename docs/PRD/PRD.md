...
- **Every drilled-in view has a visible back arrow** (item detail, import session, person/place/theme/story pages, the review surface's screens) that returns to the perceived previous view — the system/browser back and the in-app arrow must always agree (the hash router is the single source of truth; the in-app stack only re-syncs from it, never diverges).
- **The arrow names its destination** (research, 2026-08-16: iOS shows the previous title beside the chevron; the Google-Drive-style hybrid "arrow + destination" is the recommended mobile pattern). With in-app history the arrow is a plain "←" — it undoes the previous action, and the destination is wherever the user came from (NN/g). On a deep link (no in-app history) it goes **Up** to the logical parent and SAYS so: "← Family Tree", "← Places", "← Review" (Android's Up guidance: the hierarchical parent, never exiting the app).
- **Up is the escape for sideways wandering.** Back only retraces the immediate path; a user who navigates item→person→item→place for ages can always go Up to a stable parent (Android: Up navigates the hierarchy, Back navigates history).
- **Back never loses work**: mid-typing edits persist (localStorage, resumable), and a confirmation is never silently dropped by navigating back.
- **No history pollution**: only views perceived as separate pages create history entries; in-page states (an open zoom, a filtered list's internal state) do not.
- **No explicit breadcrumbs** (decision, 2026-08-16, researched): our hierarchy is shallow (home → section → detail; the review: hub → batch → document → page) and breadcrumbs consume space and clutter small screens — the research recommends back-arrows on mobile and breadcrumbs mainly for deep desktop hierarchies; the review's position line ("Page 2 of 3 · Document 2 of 3") already gives the reviewer their place. Revisit if the hierarchy deepens.
- **The browser back/forward must work well**: the app does not fight the browser's scroll restoration (no forced scroll-to-top on back/forward revisits — the user returns to where they were in a long list, not the top).
- **Refreshing the browser restores the page you're on** (user, 2026-08-16: "the browser refresh button sometimes takes you to a different page instead of refreshing the one you're on"). Any view whose position is not in the URL must be — the review surface's document and page live in the hash (`#/review/<batch>/<doc>/<page>`, kept in sync as the reviewer moves), so a refresh lands back on the same page, not on a parent list.
...
- **R1 — The review is a conversation with a hired genealogist, not a form.** The assistant reads as a professional researcher the family hired. The family never meets internal vocabulary — statuses (proposed/estimated/confirmed), "the import", "the link", process states — or third-person talk about "the reviewer" — **unless the family member themselves introduced the word**: echoing the reviewer's own vocabulary is how they understand what they're asked, and the goal is mutual understanding, not plain-language purism (user, 2026-08-15).
...
**Privacy:** **no public sharing of anything whatsoever** — all access is family-only; the mechanism is TBD (accounts, §15). Zero analytics. The app **never reads the archive directly**: all reading rides the family-only projection; the front end proposes through the sync contract, and the backend records (§7).
...
| R6 | **Privacy breach** (letters are intimate) | Archive-account authentication, no analytics; sending material to a third-party model for transcription is the family's choice to use the cloud vision model (2026-08-14: a local-only mode was available; removed 2026-09-23 as unusable for the family's pages — user) |
...
12. **ML appetite & hosting — RESOLVED (user; revised 2026-08-14, 2026-09-23):** photo-labeling (people/pets/boats), theme discovery, and **handwriting OCR for transcription drafts** are wanted; the transcription uses the vision model (cloud) — the local-only mode that was available (2026-08-14) was removed 2026-09-23 as unusable for the family's pages (the privacy posture is the family's choice of the cloud mode per document — user); imperfect suggestions are accepted with family confirmation (the propose/confirm seam).
...
14. **Full-res placement — TBD:** confirm where full-res images live (the family-only projection vs the backend's workspace) so no read path depends on the backup mirror.
...
6. **Backup = 3-2-1, export-driven.** The home workspace holds the live archive (copy 1); Google Drive is the **backup mirror**; the **annual full export** — folder download + CSV — goes to a second household and external media (copies 2–3, off-site by definition). The export ritual is load-bearing: it is the guarantee that the archive never lives only in one service. The letters and memoirs are the irreplaceable layer.
...
8. **"Within reason" boundary.** No format-migration treadmill, no exotic archival storage, no encryption that can lock out descendants. **No proprietary dependency:** the archive lives in plain, portable files (on the home disk, mirrored to Drive) — the annual export ritual guarantees it is never trapped in any service. Boring formats that have already outlived decades.
