...
| 2026-08-16 | The reviewer's ↻ is the only thing that reorients. Local display updates instantly + persists (authoritative — never reverts); the reorientation is delivered reliably to the backend, which reorients + re-OCRs + returns the updated transcription; a late backend catch-up changes OCR, never the display. No stored intent is ever replayed or auto-delivered (PRD VR10 + acceptance 11-14) | The first field test exposed defects that violated this: a stale queued intent (localStorage outbox auto-delivered on every batch load) re-oriented a page the reviewer never touched and marked its re-read failed; and the view rotation accumulated (page-02 marched to 270°/landscape) so a fix the reviewer made appeared to revert | PRD VR10 + acceptance 11-14; ux-fixes-plan "Orientation fix" |
