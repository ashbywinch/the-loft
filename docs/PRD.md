...
- **Requirement history (sanitised — family-instance facts live in the private interview records):**
  ...
  - **v0.7:** names/aliases as first-class identity; everything-to-everything graph at scale; open questions resolved (vanilla nuance, TV/show mode, ML appetite); prototype shipped — sequencing in `PLAN.md`
...
| **Show mode** | Showing, not browsing | Full-screen, no-navigation browse for showing someone the archive — big images, large type, auto-advance; operable by anyone in the room; started from any person or story (PLAN.md Slice 3 — the urgent one: elders' memory is fading) |
...
| **Show mode** | Should | The urgent one: familiar photos, shown now — elders' memory is fading (PLAN.md Slice 3) |
...
| R6 | **Privacy breach** (letters are intimate) | Archive-account authentication, no cloud dependency beyond the family's own account, no analytics |
...
6. **TV/second-screen mode — RESOLVED (via plan):** yes — gatherings are the activation metric; Show mode (Slice 3) and TV mode (Slice 6) are planned in `PLAN.md`.
...
9. **The elders and the app — RESOLVED (via plan):** yes — elders may be shown familiar photos, via Show mode (`PLAN.md` Slice 3); the family decides when, and it is never a dependency. (Sensitive; see §4.)
...
12. **ML appetite & hosting — RESOLVED (user):** photo-labeling (people/pets/boats), theme discovery, and **handwriting OCR for transcription drafts** are wanted; runs locally (on-device or the household), never a third-party cloud; imperfect suggestions are accepted with family confirmation (the propose/confirm seam).
